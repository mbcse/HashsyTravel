...
for i in range(len(res)):
    if res[i]=='RegNo:':
        print('"Reg No":','"{} {}"'.format(res[i+1],res[i+2]),end=',')
    if res[i]=='Chassis':
        print('"chassis":','"{}"'.format(res[i+2]),end=',')
    if res[i]=='Engine':
        print('"engine":','"{}"'.format(res[i+2]),end=',')          
    ...
    if res[i]=='RegDate:':
        print('"regdate":','"{}"'.format(res[i+1]),end=',')
    if res[i]=='Validity:':
