...
for i in range(len(res)):
    ...
    if res[i]=='CERTIFICATE':
        print('"Certificate No":','"{}"'.format(res[i+3]),end=',')
    
    if(res[i]=='Name:'):
           print('"Name":','"{} {}"'.format(res[i+1],res[i+2]),end=',')
    if res[i]=='Vehicle' and res[i+1]=='No:':
        print('"Vehicle No":','"{}"'.format(res[i+2]),end=',')
    if res[i]=='Issue':
        print('"issuedate":','"{}"'.format(res[i+2]),end=',')
    if res[i]=='Validity:':
        print('"Validity":','"{}"'.format(res[i+1]),end=',')
    if res[i]=='Vehicle' and res[i+1]=='Type:':
        print('"Vehicle Type":','"{}"'.format(res[i+2]),end=' ')
    if(res[i]=='Fuel:'):
           print('"Fuel":','"{}"'.format(res[i+1]),end=',')
